...
class StripeDetector:
        ...
        for _, _, file_list in os.walk(target_dir):
            for filename in file_list:
                if "[" not in filename and "]" not in filename:  # since
                    StripeDetector(os.path.join(target_dir, filename)).draw_circles_with_canny()

    def draw_circles_with_canny(self):
        edges = self.__get_edges_with_canny(thresh1=50, thresh2=200)
        points = self.__find_coordinate(edges, interval=25)
    ...
    def __find_coordinate(self, edges, interval):
        h, _ = edges.shape
        cur_h = interval
    ...
    def __draw_circles(self, points):
        new_img = self.raw_img.copy()
        for point in points:
            new_img = cv.circle(new_img, (point[0], point[1]), radius=0, color=(0, 0, 255), thickness=self.circle_thickness)
    ...
    def __merge_peak(self, peak, thresh=1):
        """
    ...
    def __weaken_shadow(self, img, save=False):
        ...
        self.__show_and_save(result_norm, "weaken-shadow", save=save)
        return result_norm

    def __enhance_contrast(self, img, save=False):
        dst = np.zeros_like(img)
        cv.normalize(img, dst, 0, 255, cv.NORM_MINMAX, cv.CV_8U)
        self.__show_and_save(dst, "enhanced-contrast", save=save)
        return dst
...
if __name__ == '__main__':
    ...
    StripeDetector("img/stripe/raw/a6.png").draw_circles_with_canny()
